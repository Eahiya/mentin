...
1.  **API Key:** This application requires a valid Google Gemini API Key.
2.  **Environment Variable:** The key must be provided via `process.env.API_KEY`.
3.  **Permissions:** The application will request **Microphone** access for the Live Mic feature (see `metadata.json`).

## 📖 Usage Guide

1.  **Run Simulation:** Click one of the scenario buttons (e.g., "Cardiac", "Fire") to start a simulated emergency call.
2.  **Monitor Triage:** Watch the *Neural Triage Intelligence* panel for real-time risk assessment and priority scoring.
3.  **Manual Takeover:** Click "Take Over" to enter Manual Mode. Type instructions into the command line to speak to the caller via Neural TTS.
4.  **Confirm Dispatch:** Once the triage is complete, click "Confirm" to deploy units.

## ⚠️ Disclaimer
*This project is a technical demonstration of AI capabilities in high-stress environments. It is NOT a production-ready emergency tool and should never be used for real-life 911 dispatch.*

---
**RAPID-100** — *Augmenting Human Response at the Speed of Thought.*
